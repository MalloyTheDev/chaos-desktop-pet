...
- touch any file **outside this project folder**.

## What it does write (and where)

As of v0.4 the app writes only inside the project folder, using **atomic**
writes (temp file then `os.replace`, so a crash can't corrupt an existing file):
...
- New code must preserve the guarantees above. Anything that would add a network
  call, a write outside the project, or a system hook is out of scope by design
